...
class Rc8Client(object):
  def __init__(self, ip_addr="192.168.0.1", port=5007, timeout=200):
  ...
  def connect(self, flag=False, speed=10, timeout=0):
    ...
    try:
      self.m_client = BCAPClient(self.ip_addr, self.port, self.timeout)
      ### start b_cap Service
  ...
  def robot_move(self, comp, pose, mode="P", interpolate="", opt=""):
    try:
      pose_ = [pose,mode,interpolste]
      self.m_client.robot_move(self.hRobot, comp, pose_, opt)
    except:
      traceback.print_exc()
      return False
    return True
  ...
  def move_joint(self, joint_value, way="@0", opt="NEXT"):
    if not self.hRobot:
      print("Error in move")
      return None
    Comp = 1
    Pose = [[float(x)  for x  in joint_value], "J", way]
    try:
      self.m_client.robot_move(self.hRobot, Comp, Pose, opt)
    except:
      traceback.print_exc()
  ...
  def convert_j_to_p(self, j_value):
    ...
    return self.m_client.robot_execute(self.hRobot, "J2P", j_value)
